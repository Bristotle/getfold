...
Circuit > Society) can exist where relevant, but nothing requires it, an independent church just has no parent.
...
Acceptance has the same bootstrap problem as creating the first church, `organization_members` INSERT requires you to already be an admin of that
...
messages per hour and is explicitly not intended for production traffic, real signups will silently fail without it.
...
keep the Manuel Technologies purple (`--color-primary`) as an **accent**, buttons, active states, links, not as a dominant surface color like
...
- [~] Member messaging (`/messages`), welcome on joining, a receipt when
      ...
      **Messages are recorded first and delivered second.** Without a
      ...
      send it later. There is **no SMS provider on the Vercel Marketplace**, `SMS_PROVIDER` selects arkesel / hubtel / twilio in
...
- [~] Mobile money via **Paystack** (Stripe, the only Marketplace payments
      ...
      - An initiated charge is **not** a contribution. Only a verified
        ...
        that did not arrive.
      - Webhook verification is HMAC-**SHA512** over the **raw** body, re-serialising the parsed JSON changes the bytes and every
...
- [ ] AI features, in rough order of moat vs. novelty:
      1. [x] Silent attrition detection (`/insights`), `attrition_watchlist()` in `0009_checkins_attrition.sql` compares
